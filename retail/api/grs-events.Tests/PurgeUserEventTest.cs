...
namespace grs_events.Tests
{
    ...
    public class PurgeUserEventTest
    {
        ...
        private static readonly string CommandLineArguments = "-c \"dotnet run -- PurgeUserEvent\"";

        [TestMethod]
        public void TestOutputPurgeUserEvent()
        {
            ...
            Assert.IsTrue(Regex.Match(consoleOutput, "(.*)Purge user event. request: (.*)\"parent\": \"projects/(.*)/locations/global/catalogs/default_catalog(.*)", RegexOptions.Singleline).Success);
            Assert.IsTrue(Regex.Match(consoleOutput, "(.*)Purge user event. request: (.*)\"filter\": (.*)\"visitorId=(.*)test_visitor_id(.*)\"(.*)", RegexOptions.Singleline).Success);
            Assert.IsTrue(Regex.Match(consoleOutput, "(.*)Purge user event. request: (.*)\"force\": true(.*)", RegexOptions.Singleline).Success);
