...
{
    [TestClass]
    public class ImportUserEventsInlineSourceTest
    {
        ...
        private static readonly string CommandLineArguments = "-c \"dotnet run -- ImportUserEventsInlineSource\"";

        [TestMethod]
        public void TestOutputImportUserEventsInlineSource()
        {
            ...
            Assert.IsTrue(Regex.Match(consoleOutput, "(.*)Import user events from inline source. request: (.*)\"parent\": \"projects/(.*)/locations/global/catalogs/default_catalog(.*)", RegexOptions.Singleline).Success);
            Assert.IsTrue(Regex.Match(consoleOutput, "(.*)Import user events from inline source. request: (.*)\"inputConfig\": (.*)\"userEventInlineSource\"(.*)", RegexOptions.Singleline).Success);
            ...
            Assert.IsTrue(Regex.Match(consoleOutput, "(.*)Number of successfully imported events:(.*)3(.*)", RegexOptions.Singleline).Success);
